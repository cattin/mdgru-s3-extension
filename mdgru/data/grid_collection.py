...
class ThreadedGridDataCollectionS3Storage(GridDataCollection):

    _defaults = {'s3_endpoint': {'help': 'The endpoint of the S3 storage', 'value': '', 'type': str},
                 'access_key': {'help': 'access key to the s3 storage', 'value': '', 'type': str},
                 'secret_key': {'help': 'Secret key to the s3 storage', 'value': '', 'type': str},
                 'data_repository': {'help': 'The repository of the LakeFS where the data are stored',
                                     'value': '', 'type': str},
    ...
    def __init__(self, w, p, location=None, tps=None, kw={}):
        ...
        w : list of int
            window size
        p : list of in
            padding size of the window
        location : str
            Location of the json file containing all filenames of the data set (create before)
        tps : paths of all samples to consider. must be provided if location is not set. (Not used in the class)
        """
        self.base_location = os.path.dirname(location)

        super(ThreadedGridDataCollectionS3Storage, self).__init__(w, p, ThreadedGridDataCollectionS3Storage.dummy, tps, kw)
        ...
        # generate a S3 client for each thread
        # for it in range(self.num_threads):
        #     self.s3_clients.append(boto3.client('s3', endpoint_url=self.s3_enpoint,
        ...
        # load the json file and parse it to the %TODO
    ...
    @staticmethod
    def dummy():
        # this is a dummy function. It is needed to trick the initial loader of MDGRU
        return "None"

    def get_all_tps(self, folder, featurefiles, maskfiles):
        # load the json file and parse it to the %TODO
        with open(folder) as f:
            self.s3_files = json.load(f)
        return [os.path.join(self.cache_path, s) for s in self.data_folders_s3]


    def save(self, data, filename, tporigin=None):

        # here we will redirect the file name to the corrent location were the json file is located
        # remember the original file name
        pure_filename = os.path.basename(filename)
        # get the top folder of the filename
        folder = os.path.basename(os.path.dirname(filename))

        # redirect the save command to the folder were the data json file is located
        folder = os.path.join(self.base_location, folder)
        if not os.path.isdir(folder):
            os.makedirs(folder)

        new_filename = os.path.join(folder, pure_filename)

        # save the file to the new location
        super(ThreadedGridDataCollectionS3Storage,self).save(data, new_filename, tporigin)
    ...
    def load(self, file, lazy=False):
        ...
        if not os.path.isdir(filename_base):
            os.makedirs(filename_base)

        feature_names = self.s3_files[os.path.basename(filename_base)]["features"] + \
                        self.s3_files[os.path.basename(filename_base)]["labels"]

        s3_filename = [match for match in feature_names if file_type in match][0]

        if not os.path.isfile(file):
            # the file is not in the cache, we need to get it from the S3 storage
            cache_filename_tmp = file + "." + str(self.randomstate.randint(100000, 999999))
            self.s3_client.download_file(self.data_repository, s3_filename, cache_filename_tmp)
            os.rename(cache_filename_tmp, file)

        return super(ThreadedGridDataCollectionS3Storage, self).load(file, lazy)


generate_defaults_info(GridDataCollection)
generate_defaults_info(ThreadedGridDataCollection)
generate_defaults_info(ThreadedGridDataCollectionS3Storage)
